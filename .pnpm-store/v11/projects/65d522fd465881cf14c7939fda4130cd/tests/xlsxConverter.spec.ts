...
describe('xlsxConverter round-trip', () => {
  ...
  it('导入及再次保存后保留 Excel 下拉选择规则', async () => {
    const data = await xlsxConverter.toUniver(
      loadFixtureBlob('data-validation-dropdown.xlsx'),
      'data-validation-dropdown.xlsx',
    )
    const validationResource = data.resources?.find(
      resource => resource.name === 'SHEET_DATA_VALIDATION_PLUGIN',
    )

    expect(validationResource).toBeDefined()
    const importedRules = Object.values(
      JSON.parse(validationResource!.data) as Record<string, Array<{ type: string; formula1?: string }>>,
    ).flat()
    expect(importedRules).toEqual(expect.arrayContaining([
      expect.objectContaining({ type: 'list', formula1: '待处理,进行中,已完成' }),
      expect.objectContaining({ type: 'list', formula1: '=$D$2:$D$4' }),
    ]))

    const exported = await xlsxConverter.toXlsx(data, 'dropdown.xlsx')
    const reparsed = await xlsxConverter.toUniver(exported, 'dropdown.xlsx')
    const reparsedResource = reparsed.resources?.find(
      resource => resource.name === 'SHEET_DATA_VALIDATION_PLUGIN',
    )
    expect(reparsedResource).toBeDefined()
    expect(Object.values(JSON.parse(reparsedResource!.data)).flat()).toHaveLength(10)
  })
})

...
async function loadExcelJs() {
  // Prefer the fork already used by the application and converter tests.
  try {
    const m = await import('@zwight/exceljs')
    return m.default ?? m
  } catch (e) {
    if (e?.code !== 'ERR_MODULE_NOT_FOUND' && e?.code !== 'MODULE_NOT_FOUND') throw e
  }
  // Fall back to the conventional resolver (works if exceljs is installed locally).
  try {
...
async function generateDataValidationDropdown(ExcelJS) {
  const wb = new ExcelJS.Workbook()
  wb.creator = 'fixture-generator'
  wb.created = new Date()
  const ws = wb.addWorksheet('下拉测试')

  ws.getRow(1).values = ['直接列表', '区域引用', '说明', '选项源']
  ws.getRow(1).font = { bold: true }
  ws.getCell('C2').value = '点击 A2:A6 或 B2:B6，确认单元格右侧显示下拉箭头'

  const priorities = ['低', '中', '高']
  priorities.forEach((value, index) => {
    ws.getCell(`D${index + 2}`).value = value
  })
  ...
  const out = path.join(FIXTURES_DIR, 'data-validation-dropdown.xlsx')
  await wb.xlsx.writeFile(out)
  console.log(`Wrote ${out}`)
}
...
const generators = [
  ...
  generateDataValidationDropdown,
]
const ExcelJS = await loadExcelJs()
fs.mkdirSync(FIXTURES_DIR, { recursive: true })
const requestedGenerator = process.argv[2]
for (const gen of generators) {
  if (requestedGenerator && gen.name !== requestedGenerator) continue
  await gen(ExcelJS)
}
if (requestedGenerator && !generators.some(gen => gen.name === requestedGenerator)) {
  throw new Error(`Unknown fixture generator: ${requestedGenerator}`)
}
console.log(requestedGenerator ? `${requestedGenerator} fixture generated.` : 'All fixtures generated.')
